document.addEventListener('DOMContentLoaded', () => {
  ...
  function fetchAndDisplayQuestions() {
    ...
    fetch(`https://raw.githubusercontent.com/varsha2k3/server/main/server.js/api/questions?name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}`)
  ...
  aptitudeForm.addEventListener('submit', event => {
    ...
    disableForm(); // Disable form after submission
  });

  // Function to disable form elements after submission
  function disableForm() {
    aptitudeForm.reset(); // Reset form fields
    const radioButtons = document.querySelectorAll('input[type="radio"]');
    radioButtons.forEach(radioButton => {
      radioButton.disabled = true;
    });
    aptitudeForm.querySelector('button[type="submit"]').disabled = true;
  }
